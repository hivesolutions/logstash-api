...
To run the custom suite of unit test for Logstash API use the following sequence of commands that will install dependencies
...
- Increment (look at `CHANGELOG.md` for semver changes) the `version` value in `setup.py` and the `VERSION` value of `src/logstash/base.py`.
...
Hive Logstash API is licensed under the [Apache License, Version 2.0](http://www.apache.org/licenses/).
